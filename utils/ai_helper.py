...
Tahlilni qisqa va aniq qil. Agar sayt xavfli bo'lsa qanday ehtiyot choralarini ko'rish kerakligini ham ayt. Markdown yulduzchalar (**) ishlatma. Faqat sof matn."""
    ...
    url_api = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key={GEMINI_API_KEY}"
            ...
            async with session.post(url_api, json=payload) as resp:
                if resp.status == 200:
                    ...
                    try:
                        ...
                        logger.warning("AI javobini qayta ishlashda xatolik.")
                        return ""
                else:
                    logger.warning(f"AI xizmati xatosi: HTTP {resp.status}")
                    return ""
