...
from utils.file_analyzer import analyze_pdf
...
router = Router(name="pdf_handler")


@router.message(F.document.file_name.lower().endswith('.pdf'))
async def handle_pdf(message: Message) -> None:
    ...
    try:
        ...
        import asyncio
        analysis = await asyncio.to_thread(analyze_pdf, file_path)
        ...
        await record_scan(user_id, "PDF", analysis['score'])
        ...
        logger.info(
            "PDF tekshirildi: %s → ball: %d (user: %d)",
            file_name, analysis['score'], user_id,
        )

    except Exception as e:
        logger.error("PDF tekshiruv xatosi: %s", e)
        try:
            await processing_msg.delete()
        except Exception:
            pass
