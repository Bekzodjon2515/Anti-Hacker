...
from utils.file_analyzer import analyze_archive
...
router = Router(name="archive_handler")

from config import SUPPORTED_EXTENSIONS
ARCHIVE_EXTENSIONS = tuple(SUPPORTED_EXTENSIONS['archive'])


@router.message(F.document, F.document.file_name.lower().endswith(ARCHIVE_EXTENSIONS))
async def handle_archive(message: Message) -> None:
    ...
    file_name = message.document.file_name or "archive.zip"

    processing_msg = await message.reply(
        f"📦 <b>Arxiv tekshirilmoqda...</b>\n"
        f"📎 Fayl: <code>{file_name}</code>\n"
        f"⏳ Ichidagi fayllar tarkibi tahlil qilinmoqda...",
        parse_mode="HTML",
    )
    ...
    try:
        ...
        import asyncio
        analysis = await asyncio.to_thread(analyze_archive, file_path, file_name=file_name)
        ...
        await record_scan(user_id, "Archive", analysis['score'])
        ...
        logger.error("Arxiv tekshiruv xatosi: %s", e)
        try:
            await processing_msg.delete()
        except Exception:
            pass
